...
public class Zad_03_UCI_14_09_2021 : MonoBehaviour
{
    ...
    //i je li se rotirao pozitivno ili negativno            DONE
    ...
    private void Update()
    {
        if (grow)
        {
            transform.localScale += new Vector3(scaleObjectX, scaleObjectY, scaleObjectZ) * Time.deltaTime;
            //g
        ...
        //h
        if(transform.localScale == transform.localScale * 2)
        {
            transform.position += new Vector3(posObjectX, posObjectY, posObjectZ);
        }
        else if(transform.localScale == transform.localScale / 2)
        {
